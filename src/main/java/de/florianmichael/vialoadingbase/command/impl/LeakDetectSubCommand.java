  ...
  public boolean execute(ViaCommandSender viaCommandSender, String[] strings) {
    if (strings.length == 1) {
      try {
        ...
        viaCommandSender.sendMessage(
            "Invalid level (" + Arrays.toString(ResourceLeakDetector.Level.values()) + ")");
      }
    } else {
      viaCommandSender.sendMessage(
          "Current leak detection level is " + ResourceLeakDetector.getLevel());
    }
    return true;
  }
