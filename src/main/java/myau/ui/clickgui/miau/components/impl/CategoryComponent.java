...
import myau.ui.clickgui.miau.animation.ScrollOffsetAnimation;
...
public class CategoryComponent {
    ...
    if (query != null && !query.trim().isEmpty()) {
      ...
      for (Module mod : myau.Myau.moduleManager.modules.values()) {
        if (mod.getName().equalsIgnoreCase("ClickGUI")
            ...
            || mod instanceof Targets) continue;
  ...
  public void render(FontRenderer renderer) {
    ...
    hoverAnim = myau.util.math.MathUtil.lerp(hoverAnim, hovering ? 1f : 0f, 0.025f * hoverDelta);
    ...
    Color themeColor1 =
        (hud != null && hud.isEnabled()) ? hud.getColor(nowMS, 0L) : theme.getFirstColor();

    cachedExtra = extra;
    cachedLiftY = liftY;
    cachedHoverAnim = hoverAnim;
    cachedRenderModuleY = renderModuleY;

    int bgColor = new Color(20, 20, 20, 200).getRGB();
  ...
  public void renderBloom(FontRenderer renderer) {
    if (cachedHoverAnim > 0.01f) {
      ...
      myau.util.render.Themes theme = myau.util.render.Themes.getCurrentTheme();
      Color themeColor1 =
          (hud != null && hud.isEnabled())
              ? hud.getColor(System.currentTimeMillis(), 0L)
              : theme.getFirstColor();
      ...
      Color ringColor =
          new Color(
              ...
              themeColor1.getGreen(),
              themeColor1.getBlue(),
              (int) (120 * cachedHoverAnim));

      GL11.glPushMatrix();
      GL11.glTranslatef(0f, -cachedLiftY, 0f);

      myau.util.shader.RoundedUtils.drawRoundOutline(
          this.renderX,
          this.renderY,
          this.width,
          cachedExtra - this.renderY,
          5f,
          1.0f,
          new Color(0, 0, 0, 0),
          ringColor);
    ...
    if (this.opened || smoothTimer != null) {
      float moduleAreaTop = this.renderY + this.titleHeight + 3 - cachedLiftY;
      float scissorBottom = cachedExtra - 2f - cachedLiftY;
      float moduleAreaHeight = Math.max(0f, scissorBottom - moduleAreaTop);
