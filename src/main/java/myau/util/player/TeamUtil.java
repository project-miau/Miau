...
public class TeamUtil {
  ...
  public static List<Entity> getLoadedEntitiesSorted() {
    ...
    return TeamUtil.mc.theWorld.loadedEntityList.stream()
        .sorted(
            (entity1, entity2) -> {
              double dist1 =
                  mc.getRenderManager()
                      .getDistanceToCamera(entity1.posX, entity1.posY, entity1.posZ);
              double dist2 =
                  mc.getRenderManager()
                      .getDistanceToCamera(entity2.posX, entity2.posY, entity2.posZ);
              if (dist1 < dist2) {
                return 1;
              }
              if (dist1 > dist2) {
  ...
  public static boolean isSameTeam(EntityPlayer player) {
    ...
    NetworkPlayerInfo selfInfo =
        mc.getNetHandler().getPlayerInfo(TeamUtil.mc.thePlayer.getUniqueID());
    if (selfInfo == null) {
      return false;
    }
  ...
  public static boolean hasTeamColor(EntityLivingBase entity) {
    if (entity == null
        || TeamUtil.mc.thePlayer == null
        || TeamUtil.mc.theWorld == null
        || TeamUtil.mc.getNetHandler() == null) {
      return false;
    }
    ...
    EntityLivingBase nearestArmorStand =
        TeamUtil.mc.theWorld.findNearestEntityWithinAABB(
  ...
  public static boolean isFriend(EntityPlayer player) {
    return player != null
        ...
        && Myau.friendManager.isFriend(player.getName());
  }

  public static boolean isTarget(EntityPlayer player) {
    return player != null
        && Myau.targetManager != null
        && Myau.targetManager.isFriend(player.getName());
  }
}
