...
/**
 * Simulated player for predicting movement ticks ahead.
 * Ported from RinBounce SimulatedPlayer.kt
...
public class SimulatedPlayer {
    ...
    private void clientPlayerLivingUpdate() {
        ...
        if (onGround && forwardSpeed >= f && !isSprinting() && flag3
                && !player.isUsingItem() && !isPotionActive(Potion.blindness) && shouldSprint) {
            setSprinting(true);
        }

        if (!isSprinting() && forwardSpeed >= f && flag3
                && !player.isUsingItem() && !isPotionActive(Potion.blindness) && shouldSprint) {
            setSprinting(true);
        }
    ...
    private void livingEntitySideMoveEntityWithHeading(float strafing, float forwards) {
        ...
        if (isInWater() && !capabilities.isFlying) {
            ...
            if (isInLava() && !capabilities.isFlying) {
                ...
                float f4 = 0.91f;
                if (onGround) {
                    f4 = worldObj.getBlockState(new BlockPos(
    ...
    private void moveEntity(double xMotion, double yMotion, double zMotion) {
        ...
        List<AxisAlignedBB> list1 = worldObj.getCollidingBoundingBoxes(player, box.addCoord(velocityX, velocityY, velocityZ));
        ...
        if (canTriggerWalking() && !flag && ridingEntity == null) {
            ...
            distanceWalkedModified = (float) ((double) distanceWalkedModified + MathHelper.sqrt_double(d12 * d12 + d14 * d14) * 0.6);
            distanceWalkedOnStepModified = (float) ((double) distanceWalkedOnStepModified + MathHelper.sqrt_double(d12 * d12 + d13 * d13 + d14 * d14) * 0.6);
    ...
    private double[] checkForCollision(double velocityX, double velocityZ) {
        ...
        for (double d6 = 0.05; velocityX != 0 && worldObj.getCollidingBoundingBoxes(player, box.offset(velocityX, -1, 0)).isEmpty(); d3 = velocityX) {
            if (velocityX < d6 && velocityX >= -d6) velocityX = 0;
            else if (velocityX > 0) velocityX -= d6;
            else velocityX += d6;
        }

        for (; velocityZ != 0 && worldObj.getCollidingBoundingBoxes(player, box.offset(0, -1, velocityZ)).isEmpty(); d5 = velocityZ) {
        ...
        for (; velocityX != 0 && velocityZ != 0
                && worldObj.getCollidingBoundingBoxes(player, box.offset(velocityX, -1, velocityZ)).isEmpty(); d5 = velocityZ) {
    ...
    private void doBlockCollisions() {
        BlockPos blockpos = new BlockPos(
                box.minX + 0.001, box.minY + 0.001, box.minZ + 0.001
        );
        BlockPos blockpos1 = new BlockPos(
                box.maxX - 0.001, box.maxY - 0.001, box.maxZ - 0.001
        );
        if (isAreaLoaded(blockpos.getX(), blockpos.getY(), blockpos.getZ(),
                blockpos1.getX(), blockpos.getY(), blockpos1.getZ(), true)) {
    ...
    private boolean handleWaterMovement() {
        if (handleMaterialAcceleration(box.expand(0.0, -0.4000000059604645, 0.0).contract(0.001, 0.001, 0.001), Material.water)) {
            fallDistance = 0.0f;
            inWater = true;
            fire = 0;
        } else {
            ...
            for (int l1 = k; l1 < l; l1++) {
                for (int i2 = i1; i2 < j1; i2++) {
                    ...
                    if (state != null && state.getBlock().getMaterial() == material) {
                        float liquidHeight = BlockLiquid.getLiquidHeightPercent(state.getValue(BlockLiquid.LEVEL));
    ...
    private boolean isAreaLoaded(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, boolean idfk) {
        int minX1 = minX;
        int minZ1 = minZ;
        int maxX1 = maxX;
        int maxZ1 = maxZ;
    ...
    private boolean isMovementBlocked() {
        return player.getHealth() <= 0f || player.sleeping;
    }

    public boolean isInLava() {
        return worldObj.isMaterialInBB(box.expand(-0.10000000149011612, -0.4000000059604645, -0.10000000149011612), Material.lava);
    }
    ...
    private boolean isLiquidPresentInAABB(AxisAlignedBB box2) {
        return worldObj.getCollidingBoundingBoxes(player, box2).isEmpty() && !worldObj.isAnyLiquid(box2);
    }
    ...
    public List<AxisAlignedBB> getCollidingBoundingBoxes(AxisAlignedBB box2) {
            ...
            for (int l1 = i1; l1 < j1; l1++) {
                if (isBlockLoaded(blockPos.set(k1, 64, l1))) {
                    for (int i2 = k - 1; i2 < l; i2++) {
                        ...
                        if (state != null) {
                            state.getBlock().addCollisionBoxesToList(worldObj, blockPos, state, box2, list, player);
                        }
                    }
                }
            }
        ...
        List<Entity> entities = worldObj.getEntitiesWithinAABBExcludingEntity(player, box2.expand(d0, d0, d0));
    ...
    private net.minecraft.entity.ai.attributes.IAttributeInstance getEntityAttribute(net.minecraft.entity.ai.attributes.IAttribute iAttribute) {
    ...
    private boolean isLivingOnLadder(Block block, World world, BlockPos pos, EntityLivingBase entity) {
