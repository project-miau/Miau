...
public class HitBox extends Module {
  ...
  public final ModeProperty showHitbox =
      new ModeProperty(
  ...
  public final BooleanProperty botCheck =
      new BooleanProperty(
          "bot-check",
          true,
          () -> this.showHitbox.getValue() == 1 || this.showHitbox.getValue() == 4);
    ...
    if (mc.getRenderViewEntity() != null && mc.theWorld != null) {
      ...
      for (Entity entity : entities) {
        if (entity.canBeCollidedWith()) {
          float collisionSize =
              (float) ((double) entity.getCollisionBorderSize() * getExpansion(entity));
          AxisAlignedBB expandedBox =
              entity.getEntityBoundingBox().expand(collisionSize, collisionSize, collisionSize);
  ...
  private boolean shouldShowEntity(EntityLivingBase entity) {
    ...
    if (!entity.ignoreFrustumCheck
        && !RenderUtil.isInViewFrustum(entity.getEntityBoundingBox(), 0.1F)) {
      return false;
    }
    ...
    if (this.isEnabled() && this.showHitbox.getValue() != 0) {
      ...
      if (!entities.isEmpty()) {
        ...
        for (EntityLivingBase entity : entities) {
          float collisionSize =
              (float) ((double) entity.getCollisionBorderSize() * this.multiplier.getValue());
          AxisAlignedBB expandedBox =
              entity.getEntityBoundingBox().expand(collisionSize, collisionSize, collisionSize);
          AxisAlignedBB offsetBox =
              new AxisAlignedBB(
                  expandedBox.minX
                      - entity.posX
                      + (RenderUtil.lerpDouble(
                              entity.posX, entity.lastTickPosX, event.getPartialTicks())
                          - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosX()),
                  expandedBox.minY
                      - entity.posY
                      + (RenderUtil.lerpDouble(
                              entity.posY, entity.lastTickPosY, event.getPartialTicks())
                          - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosY()),
                  expandedBox.minZ
                      - entity.posZ
                      + (RenderUtil.lerpDouble(
                              entity.posZ, entity.lastTickPosZ, event.getPartialTicks())
