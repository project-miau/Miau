 ...
 *
 * Allows skipping game ticks and fast-forwarding player movement
 * to approach targets faster by manipulating the client tick cycle.
 *
 * Modes:
 * - Past:   Skip ticks first (server receives fewer updates), then fast-forward
 * - Future: Fast-forward first (more movement compressed), then skip ticks
 */
public class TickBase extends Module {
    ...
    public final FloatProperty balanceRecoveryIncrement = new FloatProperty("BalanceRecoveryIncrement", 0.1f, 0.01f, 10f);
    ...
    public final FloatProperty rangeToAttackMin = new FloatProperty("RangeToAttackMin", 3.0f, 0f, 10f);
    public final FloatProperty rangeToAttackMax = new FloatProperty("RangeToAttackMax", 5.0f, 0f, 10f);
    ...
    public final ColorProperty lineColor = new ColorProperty("LineColor", Color.GREEN.getRGB(),
            () -> line.getValue());
    ...
    private enum Phase { IDLE, PAST_SKIP, PAST_ACCEL }
    ...
    private boolean isInRangeToAttack(double distance) {
        return distance >= rangeToAttackMin.getValue()
                && distance <= rangeToAttackMax.getValue();
    }
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        simulatedPlayer.rotationYaw = RotationUtil.serverYaw != 0
                ? RotationUtil.serverYaw
        ...
        int simTicks = Math.min((int) tickBalance,
                maxTicksAtATime.getValue() * (mode.getValue() == 0 ? 2 : 1));
    ...
    public void onPreTick(TickEvent event) {
        ...
        int skipTicks = Math.min(bestTick + pauseAfterTick.getValue(),
    ...
    private boolean isBlinkEnabled() {
        if (blink == null) {
            blink = (myau.module.modules.movement.Blink) Myau.moduleManager.modules.get(
                    myau.module.modules.movement.Blink.class);
        }
    ...
    private EntityLivingBase getKillAuraTarget(KillAura ka) {
        try {
            ...
            if (attackData != null) {
                return (EntityLivingBase) attackData.getClass()
                        .getMethod("getEntity")
                        .invoke(attackData);
            }
    ...
    public static class TickData {
        ...
        public TickData(Vec3 position, float fallDistance,
                        double motionX, double motionY, double motionZ,
                        boolean onGround, boolean isCollidedHorizontally) {
