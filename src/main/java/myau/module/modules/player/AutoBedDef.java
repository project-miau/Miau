...
import myau.event.impl.SwapItemEvent;
...
public class AutoBedDef extends Module {
  ...
  public void onUpdate(UpdateEvent event) {
    ...
    event.setPervRotation(
        moveFix.getValue() != 0 ? lastSteppedYaw : mc.thePlayer.rotationYaw, ROT_PRIORITY);
  }
  ...
  public void onTick(TickEvent event) {
    ...
    float useYaw =
        RotationState.isActived() && RotationState.getPriority() == ROT_PRIORITY
    ...
    mc.playerController.onPlayerRightClick(
        mc.thePlayer, mc.theWorld, held, targetBlock, targetFacing, mop.hitVec);
  ...
  private EnumFacing getBedFacing(BlockPos pos) {
    int meta =
        mc.theWorld.getBlockState(pos).getBlock().getMetaFromState(mc.theWorld.getBlockState(pos));
    switch (meta & 3) {
      case 0:
    ...
    for (EnumFacing face : EnumFacing.values()) {
      ...
      for (double u : offsets) {
        for (double v : offsets) {
          ...
          float diff =
              Math.abs(MathHelper.wrapAngleTo180_float(rot[0] - serverYaw))
  ...
  private float[] computeRotations(double dx, double dy, double dz) {
    ...
    float yaw =
        MathHelper.wrapAngleTo180_float((float) (Math.toDegrees(Math.atan2(dz, dx)) - 90.0));
  ...
  private boolean withinRotationTolerance(
      float useYaw, float usePitch, float targetYaw, float targetPitch) {
