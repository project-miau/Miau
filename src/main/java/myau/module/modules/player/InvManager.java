...
public class InvManager extends Module {
  ...
  public final IntProperty autoArmorInterval =
      new IntProperty("auto-armor-interval", 0, 0, 100, this.autoArmor::getValue);
        ...
        if (this.oDelay <= 0 && this.actionDelay <= 0) {
          if (this.isEnabled() && this.isValidGameMode()) {
            ...
            if (inventorySwordSlot == -1)
              inventorySwordSlot =
                  ItemUtil.findSwordInInventorySlot(preferredSwordHotbarSlot, false);
            ...
            if (inventoryPickaxeSlot == -1)
              inventoryPickaxeSlot =
                  ItemUtil.findInventorySlot("pickaxe", preferredPickaxeHotbarSlot, false);
            ...
            if (inventoryShovelSlot == -1)
              inventoryShovelSlot =
                  ItemUtil.findInventorySlot("shovel", preferredShovelHotbarSlot, false);
            ...
            int inventoryBlocksSlot =
                ItemUtil.findInventorySlot(preferredBlocksHotbarSlot, ItemUtil.ItemType.Block);
            ...
            if (this.autoArmor.getValue()
                && this.autoArmorTime.hasTimeElapsed(this.autoArmorInterval.getValue() * 50L)) {
              for (int i = 0; i < 4; i++) {
                ...
                if (!equippedArmorSlots.contains(i)
                  ...
                  if (stack != null) {
                    ...
                    if (isBlock
                      ...
                      this.clickSlot(
                          mc.thePlayer.inventoryContainer.windowId, this.convertSlotIndex(i), 1, 4);
                      return;
                    }
                  }
                }
