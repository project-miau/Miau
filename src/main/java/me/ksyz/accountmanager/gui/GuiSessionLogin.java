    ...
    if (button.id == 998) {
      try {
        ...
        if (session.contains(":")) { // if fully formatted string (ign:uuid:token)
          ...
          HttpURLConnection c =
              (HttpURLConnection)
                  new URL("https://api.minecraftservices.com/minecraft/profile/").openConnection();
          ...
          // get json
          JsonObject json =
              new JsonParser().parse(IOUtils.toString(c.getInputStream())).getAsJsonObject();
