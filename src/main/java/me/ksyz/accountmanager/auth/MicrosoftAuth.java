...
public final class MicrosoftAuth {
  ...
  /**
   * Silently obtains an OAuth2 authorization code from a set of Microsoft login cookies, without
   * ever opening a browser. We hit the authorize endpoint with the supplied cookies; if the session
   * is still valid Microsoft single-signs-on and replies with a redirect straight to our (never
   * contacted) localhost callback carrying the {@code code}. The code is then exchanged for tokens
   * through the regular Microsoft flow.
   *
   * @param cookies an ordered {@code name -> value} jar, e.g. from {@link
   *     me.ksyz.accountmanager.utils.CookieUtils#parse(String)}
   */
  public static CompletableFuture<String> acquireMSAuthCodeFromCookies(
      Map<String, String> cookies, Executor executor) {
    return CompletableFuture.supplyAsync(
        () -> {
            ...
            request.setEntity(
                new UrlEncodedFormEntity(
                    Arrays.asList(
                        ...
                        // We must provide the exact redirect URI that was used to obtain the auth
                        // code
                        new BasicNameValuePair(
  ...
  public static CompletableFuture<Map<String, String>> refreshMSAccessTokens(
      String msToken, Executor executor) {
    return CompletableFuture.supplyAsync(
        () -> {
          try (CloseableHttpClient client = createTrustedHttpClient()) {
            // Build a new HTTP request
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          try (CloseableHttpClient client = createTrustedHttpClient()) {
            // Build a new HTTP request
            HttpPost request =
                new HttpPost(URI.create("https://user.auth.xboxlive.com/user/authenticate"));
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          try (CloseableHttpClient client = createTrustedHttpClient()) {
            // Build a new HTTP request
            HttpGet request =
                new HttpGet(URI.create("https://api.minecraftservices.com/minecraft/profile"));
            request.setConfig(REQUEST_CONFIG);
