...
public final class OpalColorUtil {

  private OpalColorUtil() {}

  public static final int MUTED_COLOR = 0xFF808080;

  /**
   * Returns a pair of (accentColor, darkerAccent) matching Opal's getClientTheme(). Uses Miau's
   * current theme.
   */
  public static int getClientAccent() {
    Themes theme = Themes.getCurrentTheme();
    return theme.getAccentColor(new Vector2d(0, 0)).getRGB();
  }

  public static int getClientAccentDark() {
    Themes theme = Themes.getCurrentTheme();
    Color c = theme.getAccentColor(new Vector2d(0, 50));
    return darker(c.getRGB(), 0.5F);
  }
  ...
  public static int interpolateColors(final int color1, final int color2, float amount) {
    ...
    final int r = (int) MathUtil.lerp(color1RGBA[0], color2RGBA[0], amount);
    final int g = (int) MathUtil.lerp(color1RGBA[1], color2RGBA[1], amount);
    final int b = (int) MathUtil.lerp(color1RGBA[2], color2RGBA[2], amount);
    final int a = (int) MathUtil.lerp(color1RGBA[3], color2RGBA[3], amount);
    return rgbaToHex(r, g, b, a);
  }

  public static int rainbow(
      final int speed, final int index, final float saturation, final float brightness) {
    final int angle = (int) ((System.currentTimeMillis() / speed + index) % 360);
    final float hue = angle / 360f;
    return Color.HSBtoRGB(hue, saturation, brightness);
  }

  public static int interpolateColorsBackAndForth(
      final int speed, final int index, final int startColor, final int endColor) {
