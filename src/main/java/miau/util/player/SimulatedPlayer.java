...
public class SimulatedPlayer {
  ...
  private void clientPlayerLivingUpdate() {
    ...
    boolean useItemBlocksSprint = true; // NoSlow not integrated in simulation
    ...
    if (isServerWorld()) {
      ...
      if (!isInWater() || this.capabilities.isFlying) {
        if (isInLava() && !this.capabilities.isFlying) {
          ...
          if (worldObj.isRemote
              && (!worldObj.isBlockLoaded(new BlockPos(posX, 0, posZ))
  ...
  public void moveEntity(double xMotion, double yMotion, double zMotion) {
    ...
    if (noClip) {
      this.setEntityBoundingBox(
          this.getEntityBoundingBox().offset(velocityX, velocityY, velocityZ));
      resetPositionToBB();
    } else {
      double d0 = posX;
      double d1 = posY;
      ...
      List<AxisAlignedBB> list1 =
          worldObj.getCollidingBoundingBoxes(
              player, getEntityBoundingBox().addCoord(velocityX, velocityY, velocityZ));
      ...
      if (stepHeight > 0.0f && flag1 && (d3 != velocityX || d5 != velocityZ)) {
        ...
        List<AxisAlignedBB> list =
            worldObj.getCollidingBoundingBoxes(
                player, getEntityBoundingBox().addCoord(d3, velocityY, d5));
      ...
      if (canTriggerWalking() && !flag && ridingEntity == null) {
        ...
        distanceWalkedModified =
            (float) (distanceWalkedModified + MathHelper.sqrt_double(d12 * d12 + d14 * d14) * 0.6);
  ...
  public void doBlockCollisions() {
    BlockPos blockpos =
        new BlockPos(
            getEntityBoundingBox().minX + 0.001,
            getEntityBoundingBox().minY + 0.001,
            getEntityBoundingBox().minZ + 0.001);
    BlockPos blockpos1 =
        new BlockPos(
            getEntityBoundingBox().maxX - 0.001,
  ...
  public boolean isInLava() {
    return worldObj.isMaterialInBB(
        getEntityBoundingBox()
            .expand(-0.10000000149011612, -0.4000000059604645, -0.10000000149011612),
        Material.lava);
  }
  ...
  private List<Entity> getEntitiesInAABBexcluding(
      Entity entity, AxisAlignedBB bb, Predicate<Entity> predicate) {
      ...
      for (int j1 = k; j1 <= l; j1++) {
        if (isChunkLoaded(i1, j1, true)) {
          getChunkFromChunkCoords(i1, j1)
              .getEntitiesWithinAABBForEntity(entity, bb, list, predicate);
        }
      }
    }
    return list;
  ...
  public Pair<Double, Double> checkForCollision(
      SimulatedPlayer simPlayer, double velocityX, double velocityZ) {
    EntityPlayerSP player = mc.thePlayer;
    ...
    for (d6 = 0.05;
        velocityX != 0
            && worldObj
                .getCollidingBoundingBoxes(player, simPlayer.box.offset(velocityX, -1, 0))
                .isEmpty();
        d3 = velocityX) {
    ...
    for (;
        velocityZ != 0
            && worldObj
                .getCollidingBoundingBoxes(player, simPlayer.box.offset(0, -1, velocityZ))
                .isEmpty();
        d5 = velocityZ) {
    ...
    for (;
        velocityX != 0
            && velocityZ != 0
            && worldObj
                .getCollidingBoundingBoxes(player, simPlayer.box.offset(velocityX, -1, velocityZ))
                .isEmpty();
        d5 = velocityZ) {
      if (velocityX < d6 && velocityX >= -d6) {
        velocityX = 0;
