...
public class TickBase extends Module {
  ...
  private miau.module.modules.movement.Blink blink = null;

  private boolean isBlinkEnabled() {
    if (blink == null) {
      blink =
          (miau.module.modules.movement.Blink)
              Miau.moduleManager.modules.get(miau.module.modules.movement.Blink.class);
    }
  ...
  private boolean isSelected(EntityLivingBase entity) {
    ...
    if (entity instanceof EntityPlayer) {
      ...
      if (Miau.friendManager != null && Miau.friendManager.isFriend(player.getName())) return false;
    }
    return true;
  }

  private boolean canTargetInvisibles() {
    KillAura ka = (KillAura) Miau.moduleManager.modules.get(KillAura.class);
    if (ka != null) {
      try {
