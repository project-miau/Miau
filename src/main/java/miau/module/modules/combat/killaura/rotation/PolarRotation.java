...
public class PolarRotation extends RotationMode {
  ...
  public float[] processRotations(
    ...
    if (target == null) return lastRots;

    double playerSpeed = Math.hypot(mc.thePlayer.motionX, mc.thePlayer.motionZ);
    double targetSpeed = Math.hypot(target.posX - target.lastTickPosX, target.posZ - target.lastTickPosZ);
    ...
    double motionPredFactor = Math.random() * 0.7;
    ...
    double yawNoise = random.nextGaussian() * 0.15;
    double pitchNoise = random.nextGaussian() * 0.15;
    yaw += yawNoise;
    pitch += pitchNoise;
