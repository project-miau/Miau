...
import miau.mixin.IAccessorShaderGroup;
...
public class MotionBlur extends Module {
  ...
  private static final ResourceLocation MOTION_BLUR_SHADER =
      new ResourceLocation("shaders/post/miau_motion_blur.json");
  ...
  private void loadPhosphorShader() {
    ...
    try {
      ...
      ((MotionBlurShaderHook) mc.entityRenderer).miau$setMotionBlurShader(shaderGroup);
      this.reloadPhosphorIntensity();
  ...
  private void unloadPhosphorShader() {
    ...
    ShaderGroup shaderGroup = hook.miau$getMotionBlurShader();
    if (shaderGroup != null) {
      shaderGroup.deleteShaderGroup();
      hook.miau$setMotionBlurShader(null);
    }
  }

  private void reloadPhosphorIntensity() {
    ShaderGroup shaderGroup = ((MotionBlurShaderHook) mc.entityRenderer).miau$getMotionBlurShader();
    if (shaderGroup == null) {
      return;
    }
    try {
      List<Shader> shaders = ((IAccessorShaderGroup) shaderGroup).miau$getListShaders();
  ...
  private boolean isPhosphorShaderActive() {
    return OpenGlHelper.shadersSupported
        && ((MotionBlurShaderHook) mc.entityRenderer).miau$getMotionBlurShader() != null;
  }
